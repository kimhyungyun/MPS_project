...
export default function SignupForm() {
  ...
  // ✅ (1) 아이디 특수문자 안내 메시지 (아이디 input 아래 전용)
  const [mbIdError, setMbIdError] = useState('');
  ...
  const handleChange = (
    ...
    // ✅ (2) mb_id는 입력을 유지하되, 특수문자 있으면 즉시 안내 + 중복확인 초기화
    if (name === 'mb_id') {
      setIsIdChecked(false);
      setIdCheckMessage('');

      setFormData((prev) => ({ ...prev, mb_id: value }));

      if (value && !/^[a-zA-Z0-9]*$/.test(value)) {
        setMbIdError('아이디에는 특수문자를 사용할 수 없습니다. (영문/숫자만 가능)');
      } else {
        setMbIdError('');
      }
  ...
  const validateForm = () => {
    ...
    // ✅ (3) 특수문자 있으면 회원가입 진행 불가
    if (mbIdError) return setError(mbIdError), false;
  ...
  const handleCheckId = async () => {
    ...
    // ✅ (4) 특수문자 있으면 중복확인 호출 자체를 막음
    if (mbIdError) return setError(mbIdError), undefined;
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              {/* ✅ 아이디 특수문자 안내: 아이디 입력칸 바로 아래 */}
              {mbIdError && <p className="mt-1 text-xs text-red-600">{mbIdError}</p>}
            ...
            <div>
              <button
                ...
                disabled={
                  ...
                  !!mbIdError // ✅ 특수문자 있으면 가입하기 버튼도 비활성화
                }

...
const UploadButton = ({
  ...
  <label className="ml-4 bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 cursor-pointer flex items-center gap-2">
    ...
    <input
      ...
      className="hidden"
      onChange={onUpload}
      accept=".pdf,.ppt,.pptx,.hwp,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg,.gif"
    />
  </label>
);
...
const Dataroom2Page = () => {
  ...
  const fetchFiles = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await axios.get(`${apiUrl}/api/dataroom`, {
        params: {
          page: currentPage,
          search: searchTerm || undefined,
          room: ROOM, // 🔥 DATA1 / DATA2 구분
        },
        headers: {
  ...
  const handleFileUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${apiUrl}/api/dataroom/upload`,
        formData,
        {
          params: { room: ROOM }, // 🔥 DATA1 / DATA2 구분
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteFile = async (fileId: number) => {
    ...
    try {
      setIsLoading(true);
      const response = await axios.delete(`${apiUrl}/api/dataroom/${fileId}`, {
        headers: {
  ...
  const handleDownload = async (file: FileItem) => {
    ...
    try {
      const res = await axios.get(`${apiUrl}/api/dataroom/presigned`, {
